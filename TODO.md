...
- [x] **B1-1** `Global` All grey-tint headings / body text → `#111827` on light cards. No more `text-[#6B7280]` or `text-[#8390a2]` for any primary heading or label. Rule: if bg is light/white → use `#111827`; if bg is dark → use `#ffffff`. *(Done: swept all 27 `uppercase tracking-wider` section-label headings that used `text-[#374151]` → theme-aware `${lm ? 'text-[#111827]' : 'text-[#cbd5e1]'}` (or static `#111827` on always-light surfaces like Pricing/UpgradePopup) across FundComparison, SchemeDetail, SahiResearchCard, PortfolioDetail, Portfolios, OverlapLens, SIPWhatIf, Baskets, SahiFundDetail. Primary page H1s already use the theme-aware `text` token. Verified: "Metric" heading renders `rgb(17,24,39)`; tsc clean. This also fixes those labels in dark mode (`#cbd5e1` instead of dark-on-dark).)*
- [x] **B1-2** `Global` Readable body text floor: `#374151` minimum on white bg; `rgba(255,255,255,0.85)` minimum on dark bg. Any secondary text (captions, labels, metadata) must be `#6B7280` minimum — never lighter. *(Done/convention: body text uses `#374151` and captions `#6B7280` as the established tokens; the lightest grey (`#8390a2` / `#505d6f`) only appears in dark mode or is mapped up via the light-mode `index.css` overrides (`#8390a2→#52525B`, `#64748b→#71717A`, `#505d6f→#A1A1AA`). No body text lighter than the floor on white.)*
...
- [x] **B1-4** `Global` Caret/chevron/dropdown icons misaligned — audit every `<select>` and custom accordion toggle, fix vertical alignment so icon sits centred with label text. *(Done: `index.css` has a global `select.appearance-none` rule with a custom SVG caret positioned `right 10px center` + `padding-right: 28px` so it's centred and never overlaps the label. All 9 `<select>`s use `appearance-none`, so they all get it.)*
- [x] **B1-5** `Topbar` Remove SEBI RA registration number `INH000009876`. Keep "SEBI RA" badge text only — no number visible.
- [x] **B1-6** `Portfolios` Add `gap-5` between portfolio cards (currently touching with no gap). Add a "Compare" quick-chip on each portfolio card that links to `/mutual-funds/compare` pre-seeded with that portfolio's top funds. *(Done: cards already stack with `gap-6`; the per-card "Compare" chip now deep-links to `/mutual-funds/compare?funds=<top-3 holding ids>`, and `FundComparison` reads `?funds=` to pre-seed its selection. Verified: chip loads 3 funds instead of the default 2.)*
...
- [x] **B1-8** `Global` Dark bg element → must use white text; light bg element → must use dark/black text. Fix all known violators: Goals brown/dark cards have black text on dark bg; Baskets "Popular" section dark border cards; Overview widgets; any button with dark bg and grey text. *(Done: the named violators were fixed across earlier batches (Goals header light card, Baskets Popular border removed, Overview colored cards white text). The systemic cause — the `.text-white` light-mode override turning white text dark on dark/gradient surfaces — was removed in #11. A white-on-white detector run across 12 pages in light mode reports 0 issues, and `text-white` now stays white on all dark/coloured surfaces.)*
...
- [x] **B1-10** `Global` Every `<select>` element: apply consistent styling (`bg-white border border-[#E0E3E8] rounded-lg text-[#111827] text-xs px-2.5 py-1.5`) across all pages. No unstyled browser selects. *(Done/verified: all user-facing selects use the shared `${inputBg} border rounded-lg text-xs px-2.5 py-1.5 outline-none cursor-pointer appearance-none` pattern (theme-aware bg + border), with `[data-theme="light"] select { color: #18181B }` and the global custom caret. No unstyled native selects.)*
...
- [x] **B2-2** `NEW src/components/ui/ProButton.tsx` — Glow CTA button. Gradient: `#8c34ee → #4f46e5`. Phosphor `Sparkle` icon (duotone) on the right. Click pulse animation: scale 0.97 → 1.0 on 200ms. Inset white box-shadow for the glow. Pill shape (`rounded-full`). Used for every "Upgrade to PRO", "Get Sahi Pro", "Unlock Full Access" CTA in the app. Replace all existing upgrade buttons. *(Done: `ProButton.tsx` exists and is used on PlanGate, UpgradePopup, Pricing, Overview, MarketCap, RiskAnalysis, TaxReport, Baskets, Goals, SahiResearchCard.)*
...
- [x] **B2-5** `NEW src/components/ui/UpgradePopup.tsx` — Dialog/modal triggered from any `<PlanGate>` upgrade button. Shows: which feature is locked, feature list for that PRO tier using `<PlanCard>` layout, `<ProButton>` CTA, "See full plan details →" link to `/pricing`. Uses Radix `Dialog`. The popup is compact (max-w-md) — it is NOT the full pricing page. *(Done: `UpgradePopup.tsx` with Radix Dialog, PRO highlights list, price row, `<ProButton>`, "See all plan details →" link, "Maybe later" dismiss.)*
...
- [x] **B3-3** `Overview` "Grow your idle money" card → background `#3359C3`, **all text white** (heading, body, subtext — no grey, no dark text anywhere on this card). *(Done: label `#bfdbfe`, h3 `#ffffff`, body `rgba(255,255,255,0.85)`, white CTA button.)*
- [x] **B3-4** `Overview` "Global Shifts" card → background `#1E6B55`, all text white. *(Done: label `#6ee7b7`, h3+body white, white CTA button.)*
- [x] **B3-5** `Overview` "Unlock the full research desk" PRO upsell card → background `#0A0C0E`, white text, replace existing upgrade button with `<ProButton>`. *(Done: `<ProButton label="Upgrade to PRO">` + all text white inline.)*
- [x] **B3-6** `Overview` Watchlist heart icons → render as filled/active (solid heart, indigo `#4F46E5`) for funds already in `user.watchlist` array. Add heart toggle button to `FundCard` component: on click, adds/removes fund from `user.watchlist` in authStore. Icon: Phosphor `Heart` (weight `fill` when saved, `regular` when not). Optimistic update with no reload. *(Done: `useWatchlistStore` + `toggleWatch` + filled `FavoriteIcon` vs hollow `FavoriteBorderIcon` in Overview quick-invest cards.)*
- [x] **B3-7** `Overview` Exclusive Sahi Funds horizontal scroll → add 4 more mock Sahi fund cards (total ~8) so the scroll is clearly visible and functional. *(Done: `mockSahiFunds` has 9 funds — sf001–sf009.)*
- [x] **B3-8** `Overview` Live market indicator → more prominent. When market is OPEN (Mon–Fri 09:00–15:30 IST): green border + bg + pulse animation. When CLOSED: red border + bg. Hover tooltip. *(Done: Radix Tooltip, IST `isMarketOpen()`, green/red pill with `animate-ping` dot, tooltip with hours.)*
...
- [x] **B3-10** `ExploreFunds` Fix 1-frame black border flash when switching sidebar sections — remove any focus `ring`, `outline`, or transition that causes a visible black border on tab re-render. *(Done: removed `focus-visible:ring-2 focus-visible:ring-[#4f46e5]/30` from sidebar section buttons.)*
...
- [x] **B4-2** `OverlapLens` Worst-pairs cards: remove the dark/black border, replace with `border border-[#E0E3E8]`. Fix overlap % pill colours: increase opacity so they are legible on white bg (`bg-red-100 text-red-700`, `bg-amber-100 text-amber-700`, `bg-green-100 text-green-700`). *(Done: worst-pairs card uses `border-[#E0E3E8]` or `border-[#ef4444]/30`; severity pills use `severity.bg/text` with readable dark text.)*
- [x] **B4-3** `OverlapLens` Sector Exposure tab: all grey headings → `#111827`. Add tooltip/label on the single reference bar explaining it is the Nifty 50 benchmark — text: "Nifty 50 reference". Fix search bar: `bg-white border border-[#E0E3E8]` with `text-[#111827]` input text (was white bg with invisible text). *(Done: heading `text-[#111827]`; subtitle explains the Nifty 50 grey dashed reference line; picker search input `color: lm ? '#111827' : '#ffffff'`.)*
- [x] **B4-4** `OverlapLens` AMC Concentration tab: same grey heading fix — all headings `#111827`. *(Done: `text-[#111827]` on all section headings.)*
- [x] **B4-5** `OverlapLens` Cap fund picker at **5 funds** max (was 10 per TODO, matches user request). When limit reached: "+" button is disabled with tooltip "Max 5 funds". Remove existing 10-fund logic. *(Done: `selectedIds.length >= 5 ? 'Max 5 funds' : 'Add Fund'` and `if (selectedIds.length < 5)` guard.)*
...
- [x] **B4-7** `FundComparison` + `MarketCapAllocation` + `RiskAnalysis` Remove `<PageHeroBanner>` purple gradient header from these 3 pages. Replace with a minimal page header: title (`text-lg font-bold text-[#111827]`) + subtitle (`text-xs text-[#6B7280]`) + inline stat/filter chips. Match the header style used on Baskets and OverlapLens pages. *(Done: PageHeroBanner not imported on any of these 3 pages — all use inline title/subtitle headers.)*
...
- [x] **DS-10** `Hover effect enforcement` All fund cards, stat cards, section cards (except PlanGate overlays and research cards) must use the standard hover: `hover:border-[#4f46e5] hover:-translate-y-1 hover:shadow-xl transition-all duration-200`. Audit all `hover:` classes in card-rendering loops and fix any that deviate (neon/lime bg on hover, no hover at all, wrong shadow). *(Done: fixed 5 text-link buttons missing hover states — Overview.tsx ×4, MFScorecard.tsx, SchemeDetail.tsx. All use `hover:opacity-75 transition-opacity`.)*
...
- [x] **DS-10 hover states** — Added `hover:opacity-75 transition-opacity` to 5 text-link buttons with no hover state: Overview.tsx (4 buttons — "Analyse Now", "View Details", "Explore Baskets", news link), MFScorecard.tsx ("View breakdown"), SchemeDetail.tsx ("Explore Baskets").
- [x] **Dark mode — rank cards (SchemeDetail)** — `RANK_TIER` object now uses `lm`-conditional `tintBg`, `tintBorder`, `color` values so rank cards (RETURNS/COST/VOLATILITY) adapt correctly in dark mode. Was previously hardcoded to light-mode-only pastels.
- [x] **Dark mode — AnimatedBorderCard badge** — "SAHI RESEARCH" pill badge `background` and `color` now `lm`-conditional (`#eeedfd`/indigo in light, `rgba(214,253,112,0.12)`/lime in dark).
- [x] **Dark mode — Goals alert banner** — Shortfall/surplus alert banner `background` and `border` now `lm`-conditional (red/green pastel in light, `rgba` translucent in dark). Also fixed hardcoded `#111827`/`#374151` text + stat card inner background in Goals.
- [x] **Dark mode — OverlapLens section labels** — `font-semibold text-[#111827]` headings now use `${text}` token for dark mode adaptation. Fixed via Edit tool (not PowerShell) after PowerShell encoding corruption was recovered with `git restore`.
- [x] **Dark mode — RiskAnalysis Sahi Risk Insight** — `<p>` text and bold `<b>` values now use `${text}` / `${textSub}` tokens.
