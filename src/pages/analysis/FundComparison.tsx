  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className={`flex gap-1 p-1 rounded-xl ${lm ? 'bg-[#F3F4F6]' : 'bg-[#0a0c0e]'}`}>
        {TABS.map(tab => {
          const icons: Record<TabKey, React.ReactNode> = {
            overview: <CompareArrowsIcon size={12} weight="duotone" />,
            manager: <UserIcon size={12} weight="duotone" />,
            portfolio: <BarChartIcon size={12} weight="duotone" />,
            holdings: <ListIcon size={12} weight="duotone" />,
            fundinfo: <BuildingsIcon size={12} weight="duotone" />,
          }
      ...
      {activeTab === 'manager' && (
        <div className={`${card} rounded-2xl overflow-hidden`}>
          <div className={`border-b ${dividerColor}`} style={{ display: 'grid', gridTemplateColumns: cols }}>
            <div className={`px-5 py-3 text-[11px] font-semibold ${textMuted} uppercase tracking-wider`}>Fund Manager</div>
            {selectedFunds.map((f, idx) => (
              <div key={f.id} className="px-4 py-3 text-center">
                <div className="w-2 h-2 rounded-full mx-auto mb-1" style={{ background: COLORS[idx] }} />
                <p className={`text-xs font-semibold ${text} truncate`}>{f.name.split(' ').slice(0, 3).join(' ')}</p>
          ...
          {[
            ...
            <div key={row.label} className={`border-b ${rowBorder} last:border-0 ${rowHover} transition-colors`} style={{ display: 'grid', gridTemplateColumns: cols }}>
              ...
              {selectedFunds.map((f) => {
                ...
                return <div key={f.id} className="px-4 py-3 text-center">{ext ? row.render(ext) : <span className={textMuted}>—</span>}</div>
              })}
            </div>
          ))}
          ...
          <div className={`px-5 py-4 border-t ${dividerColor}`}>
            <p className={`text-[11px] font-semibold ${textMuted} uppercase tracking-wider mb-3`}>Pros & Cons</p>
      ...
      {activeTab === 'portfolio' && (
        <PlanGate requiredTier="pro" label="Portfolio Details — Sahi PRO">
          <div className={`${card} rounded-2xl overflow-hidden`}>
            <div className={`border-b ${dividerColor}`} style={{ display: 'grid', gridTemplateColumns: cols }}>
              <div className={`px-5 py-3 text-[11px] font-semibold ${textMuted} uppercase tracking-wider`}>Portfolio Metric</div>
              {selectedFunds.map((f, idx) => (
                <div key={f.id} className="px-4 py-3 text-center">
                  <div className="w-2 h-2 rounded-full mx-auto mb-1" style={{ background: COLORS[idx] }} />
                  <p className={`text-xs font-semibold ${text} truncate`}>{f.name.split(' ').slice(0, 3).join(' ')}</p>
            ...
            {/* Equity/Cash bars */}
            <div className={`px-5 py-4 border-t ${dividerColor}`}>
              <p className={`text-[11px] font-semibold ${textMuted} uppercase tracking-wider mb-3`}>Equity vs Cash Split</p>
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${selectedFunds.length}, 1fr)`, gap: '1rem' }}>
                {selectedFunds.map((f, idx) => {
                  const ext = FUND_EXTENDED[f.id]
                  if (!ext) return <div key={f.id} />
      ...
      {activeTab === 'fundinfo' && (
        <div className={`${card} rounded-2xl overflow-hidden`}>
          <div className={`border-b ${dividerColor}`} style={{ display: 'grid', gridTemplateColumns: cols }}>
            <div className={`px-5 py-3 text-[11px] font-semibold ${textMuted} uppercase tracking-wider`}>Fund Information</div>
            {selectedFunds.map((f, idx) => (
              <div key={f.id} className="px-4 py-3 text-center">
                <div className="w-2 h-2 rounded-full mx-auto mb-1" style={{ background: COLORS[idx] }} />
                <p className={`text-xs font-semibold ${text} truncate`}>{f.name.split(' ').slice(0, 3).join(' ')}</p>
          ...
          {[
            ...
            { label: 'Registrar', render: (f: typeof mockFunds[0]) => FUND_EXTENDED[f.id]?.registrar ?? '—' },
            { label: 'Custodian', render: (f: typeof mockFunds[0]) => FUND_EXTENDED[f.id]?.custodian ?? '—' },
          ].map(row => (
            <div key={row.label} className={`border-b ${rowBorder} last:border-0 ${rowHover} transition-colors`} style={{ display: 'grid', gridTemplateColumns: cols }}>
              <div className={`px-5 py-3 text-sm ${textSub}`}>{row.label}</div>
              {selectedFunds.map((f) => (
          ...
          {/* Tags */}
          <div className={`px-5 py-4 border-t ${dividerColor}`}>
            <p className={`text-[11px] font-semibold ${textMuted} uppercase tracking-wider mb-3`}>Tags</p>
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${selectedFunds.length}, 1fr)`, gap: '1rem' }}>
              {selectedFunds.map((f) => (
